...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        Cmd::Lightningd => run_lightningd().await.expect("bitcoind failed"),
        Cmd::Lnd => run_lnd().await.expect("bitcoind failed"),
        Cmd::Daemons => daemons().await.expect("bitcoind failed"),
    }
